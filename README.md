...
`chrome/utils/domQuery.ac.js` 提供额外的 `$` 函数，操作 DOM 可以方便一点；内容区下方通知由优先加载的 `VAlert` 工具提供，右侧垂直标签栏时会自动显示在左下角：
...
- `duration`: 停留毫秒数，默认 `3000`，传 `0` 或负数会保持显示直到手动关闭；鼠标悬停时暂停自动关闭，移开后按剩余时长继续计时
- `title`: 可选标题
